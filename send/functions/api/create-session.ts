...
// grava cada um no R2 e devolve uma chave curta + prazo de expiração.
// A sessão inteira expira sozinha (ver TTL_SECONDS); o worker em
// ../../cron-cleanup é a rede de segurança que apaga os objetos do R2
// depois que a entrada correspondente no KV já expirou.
...
const MAX_FILES = 20;
const MAX_TOTAL_BYTES = 200 * 1024 * 1024; // 200MB por sessão
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const r2key = `sessions/${key}/${i}-${safeName}`;
    await env.FILES.put(r2key, await file.arrayBuffer(), {
      httpMetadata: { contentType: type },
    });
    fileMetas.push({ name: safeName, r2key, size: file.size, type });
  }
